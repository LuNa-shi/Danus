...
danus/execution/
  ...
  loop.py       the round loop: kickoff prompt, run_round, pause/stop conditions, status
  processes.py  persisted process identity + exact pidfd lifecycle/reclamation
...
control files (`.status.json` `.pid` `.process.json` `.pause` `.stop` `logs/`). `agents_root` =
...
Pause and stop conditions are checked at the round boundary: `.pause`, `.stop`,
